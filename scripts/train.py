#!/usr/bin/env python
"""MegaDescriptor backbone'unu ArcFace kaybıyla kendi verimizde fine-tune eder.
...
mimarisi (Swin Transformer) katman isimlerine göre güvenilir biçimde bölünemediğinden,
iki aşamalı fine-tuning parametre sırasına göre kaba ama mimariden bağımsız bir
dondurma/açma stratejisiyle uygulanır: ilk aşamada son parametrelerin küçük bir kısmı
eğitilir, ikinci aşamada tüm ağ daha düşük bir öğrenme oranıyla açılır.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="MegaDescriptor'ı ArcFace kaybıyla fine-tune eder."
    )
...
def set_stage(model: torch.nn.Module, unfreeze_last_fraction: float) -> None:
    """Son `unfreeze_last_fraction` oranındaki parametreleri eğitime açar, öncekileri dondurur.

    Mimariden bağımsız, kaba ama güvenilir bir yaklaşım: `named_parameters()` sırası çoğu
    timm modelinde girişten çıkışa doğru olduğundan, son parametreler modelin son bloklarına
    karşılık gelir.
    """
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs):
        if not stage2_started and epoch >= args.warmup_epochs:
            print("2. aşama başlıyor: tüm parametreler açılıyor, learning-rate düşürülüyor.")
            set_stage(model, 1.0)
